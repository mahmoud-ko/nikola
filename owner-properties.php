...
class OwnerPropertiesController {
    ...
    public function create(): void {
        ...
        $amenities = is_array($input['amenities'] ?? null) ? implode(',', $input['amenities']) : ($input['amenities'] ?? '');
        ...
        $stmt->execute([
            ...
            (int)($input['stars'] ?? 5), (int)($input['rooms'] ?? 1),
            (float)($input['price_from'] ?? 0),
            $input['description'] ?? '', $amenities
        ]);
    ...
    public function update(int $id): void {
        $payload = AuthMiddleware::requireRole('owner');
        $input = json_decode(file_get_contents('php://input'), true);
        $check = $this->db->prepare("SELECT property_id FROM owner_properties WHERE property_id = ? AND owner_id = ?");
        ...
        $params[':id'] = $id;
        $this->db->prepare("UPDATE owner_properties SET " . implode(',', $fields) . " WHERE property_id = :id")->execute($params);
